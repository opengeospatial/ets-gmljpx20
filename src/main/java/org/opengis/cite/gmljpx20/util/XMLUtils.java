...
public class XMLUtils {
    ...
    /**
     * Evaluates an XPath expression using the given context and returns the result as boolean. Standard bindings (see
     * {@link NamespaceBindings#withStandardBindings()}) are used for evaluation of the XPath.
     *
     * <p>
     * <strong>Note:</strong> The Saxon implementation supports XPath 2.0 expressions when using the JAXP XPath APIs
     * (the default implementation will throw an exception).
     ...
     *             If the expression cannot be evaluated for any reason.
     */
    public static boolean evaluateXPath( Node context, String expr )
                            throws XPathExpressionException {
        return (boolean) evaluateXPath( context, expr, null, XPathConstants.BOOLEAN );
    }
     ...
     * @param nsBindings
     *            A collection of namespace bindings required to evaluate the XPath expression, where each entry maps a
     *            namespace URI (key) to a prefix (value); this may be {@code null} if not needed.
     * @return An XdmValue object representing a value in the XDM data model; this is a sequence of zero or more items,
     *         where each item is either an atomic value or a node.
     * @throws SaxonApiException
     *             If an error occurs while evaluating the expression; this always wraps some other underlying
     *             exception.
     */
    ...
    /**
     * Creates a new Element having the specified qualified name. The element must be {@link Document#adoptNode(Node)
     * adopted} when inserted into another Document.
     * 
     * @param qName
     *            A QName object.
    ...
    /**
     * Returns a List of all descendant Element nodes having the specified [namespace name] property. The elements are
     ...
     * @return A List containing elements in the specified namespace; the list is empty if there are no elements in the
     *         namespace.
     */
     ...
     * @param source
     *            A Node representing the XML source. If it is an Element node it will be imported into a new DOM
     *            Document.
    ...
    /**
     * Expands character entity ({@literal &name;}) and numeric references ( {@literal &#xhhhh;} or {@literal &dddd;})
     * that occur within a given string value. It may be necessary to do this before processing an XPath expression.
     * 
     * @param value
